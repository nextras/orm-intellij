...
class RepositoryMapperMethodMarkerProvider : RelatedItemLineMarkerProvider() {
	...
	override fun collectNavigationMarkers(
		...
		if (element !is PhpDocMethod) return

		val containingClass = element.containingClass ?: return
		val index = PhpIndex.getInstance(element.project)
		if (!OrmUtils.OrmClass.REPOSITORY.`is`(containingClass, index)) return
		if (baseMethods.contains(element.name)) return

		val repositoryClass = containingClass.fqn
		val mapperClass = repositoryClass.substring(0, repositoryClass.length - 10) + "Mapper"

		val methods = index.getClassesByFQN(mapperClass)
			.filter { OrmUtils.OrmClass.MAPPER.`is`(it, index) }
